    ...
    <div class="manager-content">
      ...
      <div class="filter-list" v-if="!showForm">
          ...
          >
            <div class="filter-info">
              <div class="filter-name">
                {{ filter.filter_name }}
                <span v-if="!filter.enabled" class="badge disabled">(desactive)</span>
              </div>
              <div class="filter-conditions">
                <span v-if="filter.from_contains">De: {{ filter.from_contains }}</span>
                <span v-if="filter.to_contains">A: {{ filter.to_contains }}</span>
                <span v-if="filter.subject_contains">Objet: {{ filter.subject_contains }}</span>
            ...
            <div class="filter-actions">
              <button @click="toggleFilter(filter)" class="btn-icon" :title="filter.enabled ? 'Desactiver' : 'Activer'">
                {{ filter.enabled ? '✓' : '○' }}
              </button>
              <button @click="editFilter(filter)" class="btn-icon" title="Modifier">
                ✏️
              </button>
              <button @click="deleteFilter(filter)" class="btn-icon" title="Supprimer">
                🗑️
              </button>
            </div>
          </div>
        ...
        <div class="list-footer">
          <button @click="applyFiltersNow" class="btn btn-secondary" :disabled="!filters.length || applying">
      ...
      <div class="filter-form" v-else>
        ...
        <div class="form-body">
          <div class="form-group">
            ...
            <input v-model="formData.filter_name" type="text" placeholder="Ex: Newsletters" />
          </div>
          ...
          <div class="form-section">
            ...
            <div class="form-group">
              ...
              <input v-model="formData.from_contains" type="text" placeholder="Ex: newsletter@" />
            </div>

            <div class="form-group">
              <label>A contient</label>
              <input v-model="formData.to_contains" type="text" placeholder="Ex: moi@example.com" />
            </div>

            <div class="form-group">
              <label>Objet contient</label>
              <input v-model="formData.subject_contains" type="text" placeholder="Ex: [SPAM]" />
            </div>
...
export default {
  ...
  methods: {
    ...
    async saveFilter() {
      ...
      const hasCondition = this.formData.from_contains || this.formData.to_contains ||
        this.formData.subject_contains || this.formData.has_attachment

      if (!hasCondition) {
        frappe.toast({ message: 'Au moins une condition est requise', indicator: 'orange' })
        return
      }

      if (this.formData.action_type === 'move' && !this.formData.target_folder) {
        frappe.toast({ message: 'Le dossier de destination est requis', indicator: 'orange' })
        return
      }
